...
sudo -E $VYOMAD_BIN --data-dir $TEST_HOME/.vyoma --socket-path /run/vyoma/test.sock --http-port 3009 > $TEST_HOME/daemon.log 2>&1 &
DAEMON_PID=$!
sleep 3
...
VM_OUTPUT=$($VYOMA run nginx:latest --hostname nginx-port-test -p "${HOST_PORT}:80" --vcpu 1 --memory 256 2>&1) || { echo "Run failed: $VM_OUTPUT"; exit 1; }
VM_ID=$(echo "$VM_OUTPUT" | grep "Success! VM ID:" | awk -F 'VM ID: ' '{print $2}' | awk '{print $1}')
